...
def read_all():
    ...
    for i in range(0, len(all_keys)):
        if all_data[all_keys[i]]["isValid"] == "False":  # edge
            ...
            source = int(all_data[all_keys[i]].get("source")[0])+217
            target = int(all_data[all_keys[i]].get("source")[1])+217
            weight = float(all_data[all_keys[i]].get("weight"))
            relation = (source, target, {"weight": weight})
            ##########
            if all_data[str(source)]["name"] != "tree" and all_data[str(target)]["name"] != "tree":
            ...
            ######
            if all_data[all_keys[i]]["name"] != "tree":
                ######
                # print(all_data[all_keys[i]]["name"])
...
def layout(position, G, n, it, fix):
    m = 0
    if fix==1:
        node_id = list(position.keys())
        fix = [node_id[-1]]
        ...
        fix = None
    while m < n:
        pos = nx.spring_layout(G, pos=position, k=12, fixed=fix, iterations=it, weight='weight')
        pos = scale(pos)
        m = m+1
    return pos
